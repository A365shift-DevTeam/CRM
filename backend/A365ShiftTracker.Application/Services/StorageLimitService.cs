...
public class StorageLimitService : IStorageLimitService
{
    ...
    public StorageLimitService(IUnitOfWork uow, ILogger<StorageLimitService> logger)
    {
        _uow = uow;
        _logger = logger;
    }
    ...
    public async Task<(bool Allowed, int Current, int Limit)> CheckLimitAsync(int userId, string module)
    {
        ...
        {
            _logger.LogError(ex, "Error in {Method}", nameof(CheckLimitAsync));
            throw;
        }
    }
    ...
    public async Task<PlanUsageDto> GetUsageAsync(int userId)
    {
        ...
        {
            _logger.LogError(ex, "Error in {Method}", nameof(GetUsageAsync));
            throw;
        }
    }
}
