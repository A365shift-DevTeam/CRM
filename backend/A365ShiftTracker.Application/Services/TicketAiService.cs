...
public class TicketAiService
{
    ...
    public TicketAiService(IHttpClientFactory httpClientFactory, IConfiguration config, ILogger<TicketAiService> logger)
    {
        _logger = logger;
    ...
    public async Task<AiGeneratedTicketDto> GenerateTicketAsync(string rawText)
    {
        ...
        {
            _logger.LogError(ex, "Error in {Method}", nameof(GenerateTicketAsync));
            throw;
        }
    }
}
