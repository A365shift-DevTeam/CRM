...
const defaultColumnIds = ['id', 'title', 'status', 'priority', 'dueDate']
...
export const ColumnManager = ({ show, onHide, columns, onColumnsChange }) => {
  ...
  const handleToggleVisibility = async (columnId) => {
    try {
      ...
      if (column) {
        await taskService.updateColumn(column.colId || column.id, {
  ...
  const handleDelete = async (columnId) => {
    if (window.confirm('Are you sure you want to delete this column? This will not delete the data, but the column will be hidden.')) {
      try {
        const column = columns.find(col => col.id === columnId)
        await taskService.deleteColumn(column?.colId || columnId)
  ...
  const handleSaveColumn = async () => {
    ...
    try {
      ...
      if (editingColumn) {
        await taskService.updateColumn(editingColumn.colId || editingColumn.id, saveData)
        const updatedColumns = columns.map(col =>
        ...
        const newColumn = await taskService.addColumn(saveData)
  ...
  const handleDragEnd = async (event) => {
    ...
    if (active.id !== over?.id) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          await taskService.reorderColumns(reorderedColumns.map(col => col.colId || col.id))
